...
26 department. Atmospheric and Oceanic Science offers most classes.
...
**Q2.** Of the departments listed in A2, which one is the most interesting to you, and why? **A2.**

**A2: Germanic Studies because it something that I wouldn't think has a connection with climate.**
...
**A3. Business and Management with 82 classes.**

```{r}
umd_courses
  courses_count <- umd_courses |>
     filter(term == 202308) |>
  group_by(department) |>
  summarize(classes = n()) |>
  arrange(desc(classes))
...
The spending increased overall over time.
...
spending_location |>
  group_by(expense_category) |>
   summerize(total_amount = sum(amount), na.rm=T)
...
1.  What was the highest amount of money the Dundalk Eagle Newspaper paid in the "maryland_expenses" dataset?

```{r}
maryland_expenses |> 
  filter(payee_name == "Dundalk Eagle Newspaper") |>
  arrange(desc(amount))


```

A6. The highest amount the Dundalk Eagle Newspaper paid was 1792 (I assume dollars).
